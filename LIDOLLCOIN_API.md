...
Snapshot diet: `GET zones` and `POST zones/action` (bearer and browser routes) forward one extra query parameter, `known`: a comma-separated list of 16-hex snapshot cache keys the game already holds. The quest service replaces those pieces with stubs, shrinking a hub snapshot from ~139 KB to ~6 KB, and the game fills them back in. For actions, `knownOnly()` forwards `known` and nothing else from the query string, so the request body stays the only command input. Regression: `node --test tests/quest-snapshot-cache.test.mjs`.
