export async function questProxy(secret,route,query,input){
 ...
 if(!['zones','zones/action','zones/inspect','cloud','cloud/action','characters/action','sprites','sprites/action','sprites/asset'].includes(route))throw Object.assign(Error('Endpoint not found.'),{status:404});
 const url=new URL(route,base);for(const k of ['character_id','sprite_id','target','controller','revision','part','history','view','bank_page'])if(query?.has(k))url.searchParams.set(k,query.get(k));
