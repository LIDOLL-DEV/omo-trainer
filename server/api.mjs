  ...
  return async (request, response, route) => {
    try {
      ...
      if(route.startsWith('social/')&&request.method==='GET') {
        ...
        if(route==='social/badges')return send(response,200,{participant,csrf,...database.social.badgePreferences(participant.id)}); // 24/7 badge setting for this account.
        ...
        if(route==='social/feed')return send(response,200,{participant,csrf,...database.social.feed(participant.id,{audience:query.get('audience')??'friends',before:query.get('before'),kind:query.get('kind')??'all'})});
        if(route==='social/stickers')return send(response,200,{participant,stickers:database.social.stickers(participant.id)}); // Sticker picker: only this member's available sticker types.
      ...
      if(route==='social/badges'&&request.method==='POST')return send(response,200,{participant,csrf,...database.social.saveBadgePreferences(participant.id,await body(request,4096))});
