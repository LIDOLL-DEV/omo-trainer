...
function buttons(){
 ...
 $('#feed-audience').disabled=feedBusy;$('#feed-kind').disabled=feedBusy;for(const id of ['feed-older','feed-newest'])$('#'+id).disabled=feedBusy;for(const n of document.querySelectorAll('#status-picture-preview input,#status-picture-preview button'))n.disabled=posting;
...
async function loadFeed(){
 ...
 try{const postId=new URLSearchParams(location.hash.split('?')[1]??'').get('post');const value=await request(postId?'post?id='+encodeURIComponent(postId):'feed?audience='+$('#feed-audience').value+'&kind='+$('#feed-kind').value+(feedBefore?'&before='+feedBefore:''));if(ticket!==epoch)return;
  ...
  $('#feed-status').textContent=value.items.length?(mobileSocial.matches?'':'Updates are visible to the audience shown on each post.'):({posts:'No posts yet. Share a little of your day.',auto:'No auto-updates yet. Turn on timeline sharing in Settings to post your logs and rolls.'}[$('#feed-kind').value]??'No updates yet. Share a little of your day.'); // Empty message matches the chosen filter.
...
for(const id of ['feed-audience','feed-kind'])$('#'+id).addEventListener('change',()=>{feedBefore=null;if(location.hash.includes('?'))location.hash='#feed';else void loadFeed();}); // Either filter restarts from the newest page.$('#feed-refresh').addEventListener('click',()=>void loadFeed());
