...
let conversationCache=[],threadOpen=false,archiveBusy=false;
...
function renderConversations(){
 ...
 for(const convo of conversationCache){
  if(mobileSocial.matches&&(!convo.latest||(folder==='inbox'&&convo.archived)||(folder==='unread'&&(convo.archived||!convo.unread))||(folder==='archived'&&!convo.archived)||!`${convo.friend.label} ${convo.latest.deleted?'':convo.latest.body}`.toLocaleLowerCase().includes(query)))continue;
  ...
  if(mobileSocial.matches){
   ...
   open.replaceChildren(el('strong',convo.friend.label),el('span',convo.latest.deleted?'Message removed':convo.latest.body,'conversation-preview'),el('small',stamp(convo.latest.created)+(convo.unread?' · '+convo.unread+' unread':''),'conversation-meta'));row.append(avatar,open);
...
function buttons(){
 ...
 $('#message-send').disabled=sending||archiveBusy||!peer||!account||!navigator.onLine;$('#message-text').disabled=sending;$('#message-friend').disabled=sending||chatBusy||archiveBusy;$('#message-new').disabled=sending||chatBusy||archiveBusy;
...
function renderPosts(posts,container,{detail=false,refresh=loadFeed,status=$('#feed-status')}={}){
  for(const post of posts){const card=el('article',undefined,'status-card'),header=el('div',undefined,'card-heading'),side=el('div',undefined,'post-meta-side'),postStatus=el('p','','field-help');postStatus.setAttribute('role','status');
   side.append(el('span',post.audience==='public'?'Public':'Friends','pill'),postMenu(post,{refresh,status,postStatus}));header.append(createIdentity(post.author,'h3'),side); // Report/Delete live in the dropdown under the audience pill.
   const posted=el('p',undefined,'field-help');posted.append(when(post.created));const copy=el('p',post.body,'social-body');card.append(header,posted,copy);if(post.body.length>400||post.body.split('\n').length>5){copy.classList.add('post-copy-collapsed');const more=button('More',()=>{const collapsed=copy.classList.toggle('post-copy-collapsed');more.textContent=collapsed?'More':'Less';more.setAttribute('aria-expanded',String(!collapsed));});more.classList.add('social-mobile-only');more.setAttribute('aria-expanded','false');card.append(more);}
...
$('#message-form').addEventListener('submit',async event=>{
 ...
 try{messageRetry??={requestId:crypto.randomUUID(),participantId:peer,body:$('#message-text').value};await request('messages',messageRetry);if(ticket!==epoch)return;messageDrafts.delete(peer);messageRetry=null;$('#message-text').value='';messageBefore=null;}
...
function postActions(post,status) {
 ...
 const top=composer({label:'Write a comment',submitLabel:'Post comment',onSubmit:body=>send(body,null)});
 ...
 function composer({label,submitLabel,onSubmit,onCancel}){
  ...
  if(onCancel)row.append(linkButton('Cancel',onCancel));form.append(caption,row);
  let retry=null;input.addEventListener('input',()=>{retry=null;}); // Editing the draft starts a fresh request ID.
  form.addEventListener('submit',async event=>{event.preventDefault();if(busy)return;busy=true;submit.disabled=true;input.disabled=true;
   try{retry??={requestId:crypto.randomUUID(),body:input.value};if(await onSubmit(retry)===false)return;input.value='';retry=null;}
   catch(error){status.textContent=error.message;}finally{busy=false;submit.disabled=false;input.disabled=false;}
  });
  return {form,input};
 } // Shared by the bottom comment box and every inline reply box.
 async function send(draft,parentId){
  const result=await request('comments',{requestId:draft.requestId,postId:post.id,body:draft.body,...(parentId?{parentId}:{})});if(ticket!==epoch)return false;
  if(!result.repeated)setCount(1);status.textContent=parentId?'Reply posted.':'Comment posted.';busy=false;await load();return true;
 ...
 function renderComment(comment,children,depth){
  ...
  if(comment.removed)card.append(el('p','Comment removed','comment-removed')); // Placeholder keeps replies attached to their thread.
   ...
   card.append(head,el('p',comment.body,'social-body'),controls);
  }
 ...
 function startReply(card,comment){
  openReply?.remove();const box=composer({label:'Reply to '+comment.author.label,submitLabel:'Reply',onSubmit:body=>send(body,comment.id),onCancel:()=>{box.form.remove();openReply=null;}}); // Only one reply box open per post.
