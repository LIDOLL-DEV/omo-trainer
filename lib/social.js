...
let account=null,epoch=0,feedBusy=false,chatBusy=false,posting=false,sending=false,preparing=false,pictures=[],postRetry=null,messageRetry=null,peer='',feedNext=null,messageBefore=null,messageNext=null;
...
function buttons(){
 ...
 $('#feed-audience').disabled=feedBusy;$('#feed-kind').disabled=feedBusy;$('#feed-older').disabled=feedBusy;for(const n of document.querySelectorAll('#status-picture-preview input,#status-picture-preview button'))n.disabled=posting;
 ...
 $('#feed-refresh').disabled=feedBusy;$('#message-refresh').disabled=chatBusy||archiveBusy;$('#feed-older').hidden=feedNext===null;$('#feed-end').hidden=feedNext!==null||!$('#status-feed').childElementCount||location.hash.includes('?post='); // Show the end note only after a real feed ran out.$('#messages-older').hidden=messageNext===null;$('#messages-newest').hidden=messageBefore===null;
 messageLayout();
}
...
function reset(){messageDrafts.clear();conversationCache=[];threadOpen=false;$('#message-search').value='';$('#message-filter').value='inbox';$('#page-messages').dataset.compose='false';memberBefore=null;clearViews();$('#status-view-post').hidden=true;$('#status-view-post').href='#feed';$('#post-status').textContent='Sign in to post an update.';account=null;pictures=[];peer='';activityBefore=null;activityNext=null;postRetry=null;messageRetry=null;messageBefore=null;$('#status-text').value='';$('#status-audience').value='friends';$('#status-pictures').value='';$('#status-picture-preview').replaceChildren();$('#message-text').value='';messageSticker(null);buttons();}
...
function recordText(record){
 if(record.kind==='roll')return {icon:'\u{1F3B2}',text:(record.result==='hold'?'rolled HOLD and held it!':'rolled PEE and had to go')+' ('+record.probability+'% chance)'}; // game die
...
function recordLine(post){
 ...
 if(post.record.kind==='roll'){ // Always say which mode was used, plus the hold streak.
  const chips=el('p',undefined,'roll-chips'),n=post.record.holdStreak??0;
  chips.append(el('span',post.record.desperationMode?'\u{1F525} Desperation mode':'Normal mode','roll-chip'+(post.record.desperationMode?' is-desperate':'')));
  if(post.record.result==='hold')chips.append(el('span',n+' hold'+(n===1?'':'s')+' in a row','roll-chip is-streak'));
  else if(n)chips.append(el('span','after '+n+' hold'+(n===1?'':'s')+' in a row','roll-chip'));
  box.append(chips);
 }
...
async function loadFeed({more=false}={}){ // more=false loads the newest page and replaces the feed; more=true appends the next older page.
 if(feedBusy||!navigator.onLine||(more&&feedNext===null))return;feedBusy=true;buttons();const ticket=epoch;
 const postId=more?null:new URLSearchParams(location.hash.split('?')[1]??'').get('post'),before=more?feedNext:null;
 $('#feed-status').textContent=more?'Loading more updates…':'Loading updates…';let loaded=false;
 try{const value=await request(postId?'post?id='+encodeURIComponent(postId):'feed?audience='+$('#feed-audience').value+'&kind='+$('#feed-kind').value+(before?'&before='+before:''));if(ticket!==epoch)return;
  const container=$('#status-feed');if(!more)clearPostGalleries(container);feedNext=postId?null:value.nextBefore; // A single linked post never pages.
  if(!more)container.replaceChildren();renderPosts(value.items,container,{detail:Boolean(postId)});
  $('#feed-status').textContent=container.childElementCount?(mobileSocial.matches?'':'Updates are visible to the audience shown on each post.'):({posts:'No posts yet. Share a little of your day.',auto:'No auto-updates yet. Turn on timeline sharing in Settings to post your logs and rolls.'}[$('#feed-kind').value]??'No updates yet. Share a little of your day.'); // Empty message matches the chosen filter.
  loaded=true;
 }catch(error){if(!more)clearPostGalleries($('#status-feed'));$('#feed-status').textContent=more?'Could not load more updates. '+error.message:error.message;}finally{feedBusy=false;buttons();}
 if(loaded&&ticket===epoch)requestAnimationFrame(fillFeed); // After a successful load only (errors never loop), keep loading while the end of the feed is still on screen (short pages, tall screens).
}
function fillFeed(){ // Load the next page when the bottom of the feed is within ~one screen of being visible.
 const more=$('#feed-more');if(feedBusy||feedNext===null||route()!=='#feed'||!more.getClientRects().length)return;
 if(more.getBoundingClientRect().top<innerHeight*2)void loadFeed({more:true});
}
if('IntersectionObserver' in window)new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting))fillFeed();},{rootMargin:'0px 0px 100% 0px'}).observe($('#feed-more')); // Infinite scroll: watch the spot just below the last post.
...
for(const id of ['feed-audience','feed-kind'])$('#'+id).addEventListener('change',()=>{feedNext=null;if(location.hash.includes('?'))location.hash='#feed';else void loadFeed();}); // Either filter restarts from the newest page.$('#feed-refresh').addEventListener('click',()=>void loadFeed());
$('#feed-older').addEventListener('click',()=>void loadFeed({more:true})); // Manual fallback for keyboards and browsers without IntersectionObserver.
...
async function loadActivity(){if(activityBusy||!navigator.onLine)return;activityBusy=true;const ticket=epoch;
 ...
 try{const value=await request('activity'+(activityBefore?'?before='+activityBefore:''));if(ticket!==epoch)return;activityNext=value.nextBefore;activityLatest=value.latest;const list=$('#activity-list');list.replaceChildren();
  ...
  for(const item of value.items){const card=el('article',undefined,'activity-card'+(item.read?'':' activity-unread'));card.append(el('h3',item.title),el('p',item.body,'social-body'),el('small',stamp(item.created)));const controls=el('div',undefined,'social-actions');
   ...
   if(item.kind==='friend-playing'){const link=el('a','Play LiDollQuest','button small secondary');link.href='#games';controls.append(link);}
