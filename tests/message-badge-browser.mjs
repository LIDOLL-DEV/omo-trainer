...
try{
 ...
 assert.deepEqual(await page.$$eval('.mobile-actions a',nodes=>nodes.map(n=>n.getAttribute('href'))),['#observation','#roll','#potty-chart','#analysis','#games','#messages']);assert.match(await page.$eval('#main-messages',n=>n.getAttribute('aria-label')),/2 unread messages/);
