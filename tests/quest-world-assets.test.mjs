...
test('quest details retain authentication and forward only bounded identity selectors',async()=>{
 assert.ok(questRoutes.has('quests/detail'));assert.equal(questScope('quests/detail','GET'),'social:read');assert.throws(()=>requireQuestMethod('quests/detail','POST'),/Method/);
 let observed;const server=createServer((req,res)=>{observed={url:req.url,authorization:req.headers.authorization};res.setHeader('Content-Type','application/json');res.end(JSON.stringify({definition:{description:'Q'.repeat(260000)}}));});await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));const previous=process.env.LIDOLLQUEST_API_URL;process.env.LIDOLLQUEST_API_URL='http://127.0.0.1:'+server.address().port+'/';
 try{const result=await questProxy('synthetic-token','quests/detail',new URLSearchParams({character_id:'char-test',quest:'guide_quest',token:'must-not-forward'}));assert.equal(result.definition.description.length,260000);assert.deepEqual(Object.fromEntries(new URL(observed.url,'http://test').searchParams),{character_id:'char-test',quest:'guide_quest'});assert.equal(observed.authorization,'Bearer synthetic-token');}finally{if(previous===undefined)delete process.env.LIDOLLQUEST_API_URL;else process.env.LIDOLLQUEST_API_URL=previous;await new Promise(resolve=>server.close(resolve));}
});
